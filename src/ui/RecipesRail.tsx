...
export function RecipesRail() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 px-1 pb-2">
      <span className="flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wider text-slate-500">
        ...
        <button
          ...
          className={cn(
            ...
            hasSelection
              ...
              : 'cursor-not-allowed bg-slate-50 text-slate-500 ring-slate-100',
          )}
        >
          <span className="max-w-[12rem] truncate">{r.label}</span>
          {r.uses > 0 && <span className="text-[9px] text-slate-500">{r.uses}×</span>}
        </button>
      ))}
